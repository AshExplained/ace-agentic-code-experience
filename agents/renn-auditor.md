...
**If no previous proof OR no `gaps:` section → INITIAL MODE:**

Set `is_re_verification = false`, proceed with Step 1.

## Step 1: Load Context (Initial Mode Only)

Gather all verification context from the stage directory and project state.
...
## Step 7.6: Security Conformance

This step fires on every audit run. Relevance filtering happens internally per checklist group via stack detection commands.
...
Extract files modified in this stage from recap.md (same technique as Step 7 anti-pattern scan):
...
These are the files to review for security issues. Store as `CHANGED_FILES`.

### 7.6.2 Auto-Scope Checklist Groups
...
### 7.6.3 LLM-Based Security Review
...
- OR security Blocker findings from Step 7.6
...
**DO NOT trust RECAP claims.** RECAPs say "implemented chat component" — you verify the component actually renders messages, not a placeholder.
