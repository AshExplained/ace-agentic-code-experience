...
For the auditor subagent, use this pattern:
...
Run these checks against each must-have artifact. Aggregate results into PROOF.md.
...
- Server lifecycle: start before gate, handle port conflicts, keep running for duration
- CLI installation: auto-install where safe, gate for user choice otherwise
- Error handling: fix broken environment before gate, never present gate with failed setup
