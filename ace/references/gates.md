<overview>
Runs execute autonomously. Gates formalize the interaction points where human verification or decisions are needed.

**Core principle:** Claude automates everything with CLI/API. Gates are for verification and decisions, not manual work.
...
**Key pattern:** Claude starts the dev server BEFORE the gate. User only needs to visit the URL.
...
**Example: Authentication Gate (Dynamic Gate)**
```xml
<task type="auto">
  <name>Deploy to Vercel</name>
  <files>.vercel/, vercel.json</files>
...
**Pattern:** Claude tries automation -> auth error -> creates gate -> you authenticate -> Claude retries -> continues
...
**Key distinction:**
- Pre-planned gate: "I need you to do X" (wrong - Claude should automate)
...
**Server stays running** for the duration of the gate. After user approves, server continues running for subsequent tasks. Only kill explicitly if:
- Run is complete and no more verification needed
- Switching to production deployment
- Port needed for different service

**Pattern:**
```xml
<!-- Claude starts server before gate -->
<task type="auto">
...
4. If no: create gate asking user to install
...
## Pre-Gate Automation Failures

**When setup fails before gate:**

| Failure | Response |
|---------|----------|
| Server won't start | Check error output, fix issue, retry (don't proceed to gate) |
| Port in use | Kill stale process or use alternate port |
| Missing dependency | Run `npm install`, retry |
| Build error | Fix the error first (this is a bug, not a gate issue) |
| Auth error | Create auth gate |
| Network timeout | Retry with backoff, then gate if persistent |

**Key principle:** Never present a gate with broken verification environment. If `curl localhost:3000` fails, don't ask user to "visit localhost:3000".
...
- Provide context: why this gate exists
...
- Mix multiple verifications in one gate (split them)
- Make verification impossible (Claude can't check visual appearance without user confirmation)

**Placement:**
- **After automation completes** - not before Claude does the work
- **After UI buildout** - before declaring stage complete
- **Before dependent work** - decisions before implementation
- **At integration points** - after configuring external services

**Bad placement:**
- Before Claude automates (asking human to do automatable work)
- Too frequent (every other task is a gate)
- Too late (gate is last task, but earlier tasks needed its result)
</writing_guidelines>
...
### Example 2: Database Setup (No Gate Needed)
...
<!-- NO GATE NEEDED - Claude automated everything and verified programmatically -->
```
...
Gates formalize human-in-the-loop points. Use them when Claude cannot complete a task autonomously OR when human verification is required for correctness.
